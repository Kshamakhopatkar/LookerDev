view: replicon_project_active_dq_error_details {
  derived_table: {
    ...
    sql:  SELECT "PROJECTACTIVE" as  interface_name, "OUTBOUND" as interface_type, rule_column , severity, error_description,company_code as ou_code ,  project_code as project_task_code, "Not available in interface" as CostTypeURI , country_of_company,  timestamp(blocked_since) as blocked_since from datacloud_adm_dq.replicon_project_active_dq_error_details
    UNION ALL
    SELECT "PROJECTACTIVE" as  interface_name, "OUTBOUND" as interface_type, " " as rule_column ," " as  severity, "DUMMY Exception inserted to handle No Exception Scenario" as error_description, " " as ou_code,  " " as Project_Code, " " as CostTypeURI ," " as  country_of_company, timestamp("1900-01-01") as  blocked_since
union all
SELECT "PROJECTTASK" as  interface_name, "OUTBOUND" as interface_type, rule_column , severity, error_description, ou_code,  project_task_code, "Not available in interface" as CostTypeURI , country_of_company, created_timestamp as blocked_since from datacloud_adm_dq.replicon_project_task_active_dq_error_details
UNION ALL
SELECT "PROJECTTASK" as  interface_name, "OUTBOUND" as interface_type," " as rule_column ," " as severity,"DUMMY Exception inserted to handle No Exception Scenario" as error_description," " as ou_code, " " as project_task_code, " " as CostTypeURI ," " as country_of_company, timestamp("1900-01-01") as blocked_since
UNION ALL
